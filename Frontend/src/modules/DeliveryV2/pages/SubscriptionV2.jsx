          ...
          <AnimatePresence>
            {activeSub ? (
              ...
              <div className="bg-white rounded-[32px] p-8 text-center border border-slate-100 shadow-sm relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="w-16 h-16 bg-red-50 rounded-2xl flex items-center justify-center mx-auto mb-4 text-[#FF6A00]">
          ...
          <div className="relative bg-white rounded-[32px] p-6 border border-slate-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between mb-6">
              ...
              <Button 
                ...
                className="bg-[#FF6A00] hover:bg-[#E64D02] text-white rounded-2xl px-5 py-2.5 h-auto flex items-center gap-2 text-xs font-black shadow-lg shadow-orange-500/20 active:scale-95 transition-all"
              >
            ...
            <div className="grid gap-4">
              {sortedPlans.map((plan) => {
                ...
                return (
                  <div 
                    key={plan._id}
                    className={`relative bg-white rounded-[28px] p-5 border-2 transition-all duration-300 ${isCurrentPlan ? 'border-[#FF6A00] shadow-xl shadow-orange-500/5' : 'border-slate-50 hover:border-slate-200 shadow-sm'}`}
                  >
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-colors ${isCurrentPlan ? 'bg-[#FF6A00] text-white shadow-lg shadow-orange-500/20' : 'bg-slate-50 text-slate-400'}`}>
                          {getPlanIcon(plan.durationUnit)}
                        </div>
                        <div>
                          <h4 className="font-black text-slate-900 tracking-tight">{plan.name}</h4>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[#FF6A00] font-black text-xl">₹{plan.price}</span>
                      ...
                      {plan.durationUnit === 'DAY' ? (
                        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 max-w-[140px] relative overflow-hidden group/tip">
                          <div className="flex items-start gap-2 relative z-10">
                            <Info className="w-3.5 h-3.5 text-slate-400 group-hover/tip:text-[#FF6A00] transition-colors shrink-0 mt-0.5" />
                            <div>
            ...
            >
              ...
              <div className="px-6 pt-8 pb-10">
                ...
                <div className="flex items-center gap-4 mb-8">
                  <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center text-[#FF6A00] shrink-0 border border-red-100">
                  ...
                  <div className="space-y-3">
                    <div className="relative group">
                      <div className="absolute left-5 top-1/2 -translate-y-1/2 flex items-center pointer-events-none">
                        <span className="text-xl font-black text-slate-300 group-focus-within:text-[#FF6A00] transition-colors">₹</span>
                      </div>
                      <input 
                        type="number" 
                        value={topupAmount} 
                  ...
                  <div className="flex gap-2">
                    {quickAmounts.map(amt => (
                      <button 
                        ...
                        className={`flex-1 py-3 rounded-xl text-[11px] font-black transition-all border ${topupAmount === String(amt) ? 'bg-[#FF6A00] border-[#FF6A00] text-white shadow-lg shadow-orange-500/20' : 'bg-white border-slate-100 text-slate-500 hover:border-red-200 hover:text-red-600'}`}
                      >
                        +₹{amt}
                      </button>
                    ))}
