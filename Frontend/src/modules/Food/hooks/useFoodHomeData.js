  ...
  useEffect(() => {
    ...
    const fetchBootstrap = async () => {
      ...
      if (results[0].status === "fulfilled") {
        ...
        // One slot per banner, even when the row has no artwork — the carousel
        // pairs images[i] with data[i], and filtering here would shift the copy
        // onto the wrong slide. Empty slots render the gradient fallback.
        const imgs = list.map(b => (typeof b?.imageUrl === "string" ? b.imageUrl : ""));
