...
import { VegMark, PriceRow, PageHeader } from "@food/components/user/swiggy"
const debugLog = (...args) => { }
...
Order again from this restaurant in the ${companyName} app.`
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-(--sw-surface-alt) pb-10">
        <PageHeader title="Your Orders" onBack={() => navigate("/user")} />
    ...
    return (
      <div className="min-h-screen bg-(--sw-surface-alt) pb-10">
        ...
        <div className="px-4 py-8 text-center">
          <p className="text-(--sw-text-secondary)">You haven't placed any orders yet</p>
          <Link to="/user">
  ...
  return (
    <div className="min-h-screen bg-(--sw-surface-alt) pb-10 font-sans">
      {/* Header */}
      <PageHeader title="Your Orders" onBack={() => navigate("/food/user")} />

      {/* Search Bar */}
      <div className="p-4 bg-(--sw-surface) mt-1">
        <div className="flex items-center bg-(--sw-surface) border border-(--sw-border) rounded-lg px-3 py-2 shadow-sm">
          ...
          <input
            ...
            className="flex-1 ml-3 outline-none bg-transparent text-(--sw-text-secondary) placeholder-gray-400 dark:placeholder-gray-500"
          />
        </div>
      </div>
      ...
      <div className="px-4 py-2 space-y-4">
        {filteredOrders.length === 0 ? (
          <div className="bg-(--sw-surface) rounded-xl shadow-sm border border-(--sw-border) p-8 text-center">
            <p className="text-(--sw-text-secondary)">No orders found matching your search</p>
          </div>
        ) : (
          filteredOrders.map((order) => {
            ...
            return (
              <div key={order.id} className="relative bg-(--sw-surface) rounded-xl shadow-sm border border-(--sw-border) overflow-hidden">
                {/* Card Header: Restaurant Info */}
                <div className="flex items-start justify-between p-4 pb-2">
                  <div className="flex gap-3">
                    {/* Restaurant Image */}
                    <div className="w-14 h-14 rounded-lg bg-(--sw-surface-sunken) overflow-hidden flex-shrink-0">
                      <img
                        src={restaurantImage}
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-(--sw-text) text-lg leading-tight">{order.restaurant}</h3>
                      <p className="text-[11px] text-(--sw-text-muted) mt-0.5">
                        Order ID: <span className="font-semibold text-(--sw-text-secondary)">{order.orderId || order.id}</span>
                      </p>
                      <p className="text-xs text-(--sw-text-muted) mt-0.5">{location}</p>
                      ...
                      {order.restaurantId && (
                        <Link to={`/user/restaurants/${order.restaurantId}`}>
                          <button className="text-xs text-primary font-medium flex items-center mt-1 hover:text-primary">
                  ...
                  <button
                    ...
                    className="p-1 rounded-full hover:bg-(--sw-surface-alt) transition-colors"
                  >
                    <MoreVertical className="w-5 h-5 text-(--sw-text-muted)" />
                  </button>
                </div>

                {/* Three-dots dropdown menu */}
                {activeMenuOrderId === order.id && (
                  <div className="absolute right-3 top-10 z-20 w-40 rounded-xl bg-(--sw-surface) shadow-lg border border-(--sw-border) py-1 text-xs">
                    <button
                      type="button"
                      onClick={() => handleShareRestaurant(order)}
                      className="w-full text-left px-3 py-2 hover:bg-(--sw-surface-alt) text-(--sw-text)"
                    >
                      Share restaurant
                    </button>
                    <button
                ...
                {/* Separator */}
                <div className="border-t border-dashed border-(--sw-border) mx-4 my-1"></div>
                ...
                <div className="px-4 py-2 space-y-2">
                  {order.items && order.items.length > 0 ? (
                    order.items.map((item, idx) => {
                      ...
                      return (
                        <div key={item._id || item.id || item.itemId || idx} className="flex items-start gap-3">
                          {/* Item Image */}
                          {itemImage && (
                            <div className="w-12 h-12 rounded-lg bg-(--sw-surface-alt) overflow-hidden flex-shrink-0">
                              <img
                                src={itemImage}
                                alt={itemName}
                                className="w-full h-full object-cover"
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start gap-2">
                              {/* Veg/Non-Veg Icon */}
                              <VegMark type={isVeg ? "veg" : "nonveg"} size="lg" className="mt-0.5" />
                              <div className="flex-1 min-w-0">
                                <span className="text-sm text-(--sw-text) font-medium block">
                                  {itemQuantity} x {itemName}
                                </span>
                                {item.variantName ? (
                                  <p className="text-xs text-(--sw-text-muted) mt-0.5">{item.variantName}</p>
                                ) : null}
                                {item.description && (
                                  <p className="text-xs text-(--sw-text-muted) mt-0.5 line-clamp-1">{item.description}</p>
                                )}
                              </div>
                              <div className="text-right flex-shrink-0">
                                <span className="text-sm font-semibold text-(--sw-text)">{"\u20B9"}{itemTotal.toFixed(2)}</span>
                                {itemQuantity > 1 && (
                                  <p className="text-xs text-(--sw-text-muted)">{"\u20B9"}{itemPrice.toFixed(2)} each</p>
                                )}
                              </div>
                            </div>
                          </div>
                    ...
                    <p className="text-sm text-(--sw-text-muted)">No items found</p>
                  )}
                </div>

                {/* Order Summary Section */}
                <div className="px-4 py-3 bg-(--sw-surface-alt) rounded-lg mx-4 mb-2">
                  <div>
                    {order.subtotal > 0 && <PriceRow label="Subtotal" value={order.subtotal} />}
                    {order.deliveryFee > 0 && (
                    ...
                    {order.pricing?.couponCode && (
                      <PriceRow label="Coupon Applied" value={order.pricing.couponCode} />
                    )}
                    <PriceRow label="Total" value={order.total} tone="total" />
                  </div>
                </div>
                ...
                <div className="px-4 py-2 flex items-center justify-between">
                  <div className="flex-1">
                    <p className="text-xs text-(--sw-text-muted)">Order placed on {formatDate(order.createdAt)}</p>
                    {order.deliveredAt && (
                      <p className="text-xs text-(--sw-text-muted) mt-0.5">Delivered on {formatDate(order.deliveredAt)}</p>
                    )}
                    {order.payment && (
 ...
 'bg-(--sw-surface-alt) text-(--sw-text-secondary)'
                    ...
                    {isUserCancelled && (
                      <p className="text-xs font-medium text-(--sw-text-muted) mt-1">Cancelled by you</p>
                    )}
                    {isCancelled && !isRestaurantCancelled && !isUserCancelled && (
                      <p className="text-xs font-medium text-(--sw-text-muted) mt-1">Cancelled</p>
                    )}
                  ...
                  <div className="flex items-center ml-4">
                    <Link to={`/user/orders/${order.id}`}>
                      <button className="text-xs text-primary font-medium hover:text-primary flex items-center gap-1">
                        View Details
                ...
                {/* Separator */}
                <div className="border-t border-(--sw-border) mx-4"></div>
                ...
                <div className="px-4 py-3 flex items-center justify-between">
                  ...
                  {isRestaurantCancelled ? (
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-xs text-(--sw-text-secondary) ml-7">Refund will be processed in 24-48 hours</p>
                    </div>
                  ) : needsPayment ? (
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        ...
                        className="ml-1 text-xs font-bold text-primary hover:text-primary"
                      >
                        Pay now →
                      </Link>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm text-(--sw-text)">You rated</span>
                    ...
                    <div>
                      <p className="text-xs text-(--sw-text-muted)">Order delivered</p>
                      <button
                        type="button"
                    ...
                    <div>
                      <p className="text-xs text-(--sw-text-muted)">{order.status === 'preparing' ? 'Preparing' : order.status === 'outForDelivery' ? 'Out for delivery' : order.status === 'confirmed' ? 'Order confirmed' : order.status === 'scheduled' ? 'Scheduled' : ''}</p>
                      {/* Countdown Timer */}
                  ...
                  {isDelivered && !paymentFailed && (
                    <button
                      ...
                      className="bg-primary hover:bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-1 shadow-sm transition-colors"
                    >
      ...
      <div className="flex justify-center mt-8 mb-4">
        <h1 className="text-4xl font-bold text-gray-200 dark:text-gray-700 tracking-tighter italic">Tiffingo</h1>
      </div>

      {/* Rating & Feedback Modal */}
      {ratingModal.open && ratingModal.order && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-3xl bg-(--sw-surface) shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="px-6 py-6">
              <div className="mb-6">
                <p className="text-sm font-semibold text-(--sw-text) mb-3">
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border-2 border-(--sw-border) px-4 py-2 text-sm text-(--sw-text) placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary resize-none transition-all"
              ...
              <button
                ...
                className="w-full rounded-xl bg-gradient-to-r from-primary to-primary text-white text-base font-bold py-3.5 hover:from-primary hover:to-primary disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg ring-red-500/30 flex items-center justify-center gap-2"
              >
                {submittingRating ? (
                  <>
      ...
      {showShareModal && sharePayload && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 px-4 pb-4 pt-10 sm:items-center">
          <div className="w-full max-w-sm rounded-3xl bg-(--sw-surface) shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between border-b border-(--sw-border) px-5 py-4">
              <div>
                <h3 className="text-base font-semibold text-(--sw-text)">Share restaurant</h3>
                <p className="text-xs text-(--sw-text-muted) mt-0.5">Native share available ho to sab supported apps wahan dikhenge</p>
              </div>
              <button
                type="button"
                onClick={() => setShowShareModal(false)}
                className="rounded-full p-2 text-(--sw-text-muted) hover:bg-(--sw-surface-alt)"
            ...
            <div className="px-5 py-5 space-y-3">
              {typeof navigator !== "undefined" && navigator.share && (
                <button
                  ...
                  className="w-full rounded-2xl bg-primary px-4 py-3 text-sm font-semibold text-white flex items-center justify-center gap-2 hover:bg-primary transition-colors"
                >
              ...
              <div className="grid grid-cols-3 gap-3">
                <button
                  ...
                  className="rounded-2xl border border-(--sw-border) px-3 py-4 text-xs font-medium text-(--sw-text-secondary) flex flex-col items-center gap-2 hover:bg-(--sw-surface-alt)"
                >
                ...
                <button
                  ...
                  className="rounded-2xl border border-(--sw-border) px-3 py-4 text-xs font-medium text-(--sw-text-secondary) flex flex-col items-center gap-2 hover:bg-(--sw-surface-alt)"
                >
                  <Link2 className="w-5 h-5 text-(--sw-text)" />
                  X
                </button>
                <button
                  type="button"
                ...
                <button
                  ...
                  className="rounded-2xl border border-(--sw-border) px-3 py-4 text-xs font-medium text-(--sw-text-secondary) flex flex-col items-center gap-2 hover:bg-(--sw-surface-alt)"
                >
                  <Copy className="w-5 h-5 text-(--sw-text-secondary)" />
                  Copy link
                </button>
              </div>
            </div>
