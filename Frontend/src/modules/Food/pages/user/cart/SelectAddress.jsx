...
// Read through to the token rather than repeating the literal, so this page
// follows the Food palette (and its dark variant) like the rest of the flow.
const BRAND_RED = "var(--sw-primary)"
          ...
          <ScrollReveal delay={0.05}>
            <Card>
              ...
              <CardContent>
                ...
                <p className="mt-2 text-xs text-(--sw-text-muted)">
              ...
              <CardContent>
                <div className="relative">
                  <Input
                    ...
                    className="pr-10"
                  />
                  <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-(--sw-text-muted)" />

                  {showDropdown && (
                    <div className="absolute z-20 mt-2 w-full rounded-xl border border-(--sw-border) bg-(--sw-surface) shadow-xl overflow-hidden">
                      {suggestions.length === 0 ? (
                        <div className="p-4 text-sm text-(--sw-text-secondary)">
                          No matches found.
                        </div>
                      ) : (
                          ...
                          {suggestions.map((addr) => {
                            ...
                            return (
                              <button
                                ...
                                className="w-full text-left px-4 py-3 hover:bg-red-50/60 transition-colors border-b border-(--sw-border) last:border-b-0"
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0">
                                    <div className="flex items-center gap-2">
                                      <p className="font-semibold text-(--sw-text)">
                                    ...
                                    <p className="text-sm text-(--sw-text-secondary) truncate">
