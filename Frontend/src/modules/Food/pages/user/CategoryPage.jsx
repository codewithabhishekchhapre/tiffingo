...
export default function CategoryPage() {
  ...
  return (
    <div className={`min-h-screen bg-(--sw-surface) ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
      {/* Sticky Header */}
      <div className="sticky top-0 z-20 bg-white/95 dark:bg-[#1a1a1a]/95 backdrop-blur supports-[backdrop-filter]:bg-white/90 shadow-sm">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2 px-3 md:px-6 py-3 border-b border-(--sw-border)">
            <button
              onClick={() => navigate('/user')}
              className="w-9 h-9 flex items-center justify-center hover:bg-(--sw-surface-alt) rounded-full transition-colors flex-shrink-0"
            >
              <ArrowLeft className="h-5 w-5 text-(--sw-text-secondary)" />
            </button>
            ...
            <div className="flex-1 relative max-w-2xl">
              ...
              <Input
                ...
                className="pl-10 pr-4 h-11 md:h-12 rounded-lg border-(--sw-border-strong) bg-(--sw-surface-alt) focus:bg-(--sw-surface) dark:focus:bg-[#2a2a2a] focus:border-(--sw-border-strong) text-sm md:text-base dark:text-white placeholder:text-gray-600 dark:placeholder:text-gray-400"
              />
            </div>
          </div>
          ...
          <div
            ...
            className="flex gap-4 md:gap-6 overflow-x-auto scrollbar-hide px-4 md:px-6 py-3 bg-(--sw-surface) border-b border-(--sw-border)"
            style={{
              scrollbarWidth: "none",
            ...
            {showCategorySkeleton ? (
              ...
              categories && categories.length > 0 ? categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    {isAllCategory ? (
                      <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full border-2 transition-all flex items-center justify-center ${isSelected ? 'border-primary shadow-lg bg-[#FFF5F5] dark:bg-primary/20' : 'border-(--sw-border) bg-(--sw-surface) '}`}>
                        <Grid2x2 className={`h-6 w-6 md:h-7 md:w-7 ${isSelected ? 'text-primary' : 'text-(--sw-text-muted)'}`} />
                      </div>
                    ) : cat.image ? (
                  <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-2 transition-all ${isSelected ? 'border-primary shadow-lg' : 'border-transparent'
                      ...
                      <div
                        className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-(--sw-surface-alt) flex items-center justify-center border-2 transition-all ${isSelected ? 'border-primary shadow-lg bg-[#FFF5F5] dark:bg-primary/20' : 'border-transparent'
 }`}
                        aria-label={`${cat.name} category`}
                      >
                        <span className="text-sm md:text-base font-semibold text-(--sw-text-secondary)">
                          {String(cat.name || "?").trim().slice(0, 2).toUpperCase()}
                        </span>
                      </div>
                    )}
                    <span className={`text-xs md:text-sm font-medium whitespace-nowrap ${isSelected ? 'text-primary dark:text-primary' : 'text-(--sw-text-secondary)'
 }`}>
                      {cat.name}
                    </span>
                  </button>
                ...
                <div className="flex items-center justify-center py-4">
                  <span className="text-sm text-(--sw-text-secondary)">No categories available</span>
                </div>
              )
            )}
          </div>
          ...
          <div className="flex flex-col md:flex-row md:flex-wrap gap-2 px-4 md:px-6 py-3">
            ...
            >
              <Button
                ...
                className="h-7 md:h-8 px-2.5 md:px-3 rounded-md flex items-center gap-1.5 whitespace-nowrap shrink-0 transition-all bg-(--sw-surface) border border-(--sw-border) hover:bg-(--sw-surface-alt)"
              >
                <SlidersHorizontal className="h-3.5 w-3.5 md:h-4 md:w-4" />
                <span className="text-xs md:text-sm font-bold text-(--sw-text)">Filters</span>
              </Button>
              {[
                { id: 'under-30-mins', label: 'Under 30 mins' },
 ...
 : 'bg-(--sw-surface) border border-(--sw-border) hover:bg-(--sw-surface-alt)'
 }`}
                  >
                    <span className={`text-xs md:text-sm text-(--sw-text) font-bold ${isActive ? 'text-white' : 'text-(--sw-text)'}`}>{filter.label}</span>
                  </Button>
                )
              })}
            </div>
 ...
 : 'bg-(--sw-surface) border border-(--sw-border) hover:bg-(--sw-surface-alt)'
                  ...
                  >
                    {Icon && <Icon className={`h-3.5 w-3.5 md:h-4 md:w-4 ${isActive ? 'text-white' : 'text-(--sw-text)'}`} />}
                    <span className={`text-xs md:text-sm font-bold ${isActive ? 'text-white' : 'text-(--sw-text)'}`}>{filter.label}</span>
                  </Button>
                )
              })}
            </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4 sm:py-6 md:py-8 lg:py-10 space-y-6 md:space-y-8 lg:space-y-10">
        <div className="max-w-7xl mx-auto">
          ...
          {filteredRecommended.length > 0 && selectedCategory !== 'all' && (
            <section>
              <h2 className="text-xs sm:text-sm md:text-base font-semibold text-(--sw-text-muted) tracking-widest uppercase mb-4 md:mb-6">
                RECOMMENDED FOR YOU
              </h2>
                      ...
                      <div className={`group ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
                        ...
                        <div className="relative aspect-square rounded-xl md:rounded-2xl overflow-hidden mb-2">
                          ...
                          {restaurant.categoryDishImage ? (
                            <img
                              ...
                              onError={(e) => {
                                ...
                                if (restaurant.image) {
                                  ...
                                  placeholder.className = 'w-full h-full flex items-center justify-center bg-(--sw-surface-alt) text-6xl'
                            ...
                            <div className="w-full h-full flex items-center justify-center bg-(--sw-surface-alt) text-6xl">
                              ???
                            </div>
                          )}
                          ...
                          {/* Offer Badge */}
                          {restaurant.offer && (
                            <div className="absolute top-1.5 left-1.5 bg-gradient-to-r from-primary to-primary text-white text-[10px] md:text-xs font-semibold px-1.5 py-0.5 rounded shadow-sm">
                              {restaurant.offer}
                            </div>
                          )}
                          ...
                          <RatingPill
                            ...
                            size="sm"
                            className="absolute bottom-0 left-0 border-[4px] border-(--sw-surface)"
                          />
                        </div>

                        <h3 className="font-semibold text-(--sw-text) text-xs md:text-sm line-clamp-1">
                          {isCategoryView ? (restaurant.categoryDishName || restaurant.featuredDish || restaurant.name) : restaurant.name}
                        </h3>
                        {isCategoryView && (
                          <p className="text-[10px] md:text-xs text-(--sw-text-muted) line-clamp-1">
                            {restaurant.name}
                          </p>
                        )}
                        {restaurant.deliveryTime && (
                          <div className="flex items-center gap-1 text-(--sw-text-muted) text-[10px] md:text-xs">
              ...
              {filteredAllRestaurants.map((restaurant) => {
                ...
                return (
                  <Link key={restaurant.id} to={`/user/restaurants/${restaurantSlug}`} className="h-full flex">
                    <Card className={`overflow-hidden cursor-pointer gap-0 border-0 dark:border-gray-800 group bg-(--sw-surface) shadow-md hover:shadow-xl transition-all duration-300 py-0 rounded-md h-full flex flex-col w-full ${shouldShowGrayscale ? 'grayscale opacity-75' : ''
 }`}>
                      {/* Image Section */}
                      <div className="relative h-44 sm:h-52 md:h-60 lg:h-64 xl:h-72 w-full overflow-hidden rounded-t-md flex-shrink-0">
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="absolute top-3 right-3 h-9 w-9 md:h-10 md:w-10 bg-white/90 dark:bg-[#1a1a1a]/90 backdrop-blur-sm rounded-lg hover:bg-(--sw-surface) dark:hover:bg-[#2a2a2a] transition-colors"
                          onClick={(e) => {
                            e.preventDefault()
                            e.stopPropagation()
                        ...
                        >
                          <Bookmark className={`h-5 w-5 md:h-6 md:w-6 ${isFavorite ? "fill-gray-800 dark:fill-gray-200 text-(--sw-text)" : "text-(--sw-text-secondary)"}`} strokeWidth={2} />
                        </Button>
                      </div>
                      ...
                      <CardContent className="p-3 sm:p-4 md:p-5 lg:p-6 gap-0 flex-1 flex flex-col">
                        ...
                        <div className="flex items-start justify-between gap-2 mb-2 lg:mb-3">
                          <div className="flex-1 min-w-0">
                            <h3 className="text-md md:text-xl lg:text-2xl font-bold text-(--sw-text) line-clamp-1 lg:line-clamp-2">
                              {isCategoryView ? (restaurant.categoryDishName || restaurant.featuredDish || restaurant.name) : restaurant.name}
                            </h3>
                            {isCategoryView && (
                              <p className="mt-1 text-sm md:text-base text-(--sw-text-muted) line-clamp-1">
                                {restaurant.name}
                              </p>
                            )}
                          </div>
                          <RatingPill value={restaurant.rating} size="lg" className="shrink-0" />
                        </div>

                        {/* Delivery Time & Distance */}
                        {(restaurant.deliveryTime || restaurant.distance) && (
                          <div className="flex items-center gap-1 text-sm md:text-base lg:text-lg text-(--sw-text-muted) mb-2 lg:mb-3">
                            {restaurant.deliveryTime && (
                              <>
                        ...
                        {restaurant.offer && (
                          <div className="flex items-center gap-2 text-sm md:text-base lg:text-lg mt-auto">
                            ...
                            <span className="text-(--sw-text-secondary) font-medium">{restaurant.offer}</span>
                          </div>
                        )}
                      </CardContent>
                    </Card>
          ...
          <AnimatePresence>
            {isFilterOpen && (
              <div className="fixed inset-0 z-[100]">
                ...
                {/* Modal Content */}
                <div className="absolute bottom-0 left-0 right-0 md:left-1/2 md:right-auto md:-translate-x-1/2 md:max-w-4xl bg-(--sw-surface) rounded-t-3xl md:rounded-3xl max-h-[85vh] md:max-h-[90vh] flex flex-col animate-[slideUp_0.3s_ease-out]">
                  {/* Header */}
                  <div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-(--sw-border)">
                    <h2 className="text-lg md:text-xl font-bold text-(--sw-text)">Filters and sorting</h2>
                    <button
                      onClick={() => {
                        setIsLoadingFilterResults(true)
                        setActiveFilters(new Set())
                  ...
                  <div className="flex flex-1 overflow-hidden">
                    {/* Left Sidebar - Tabs */}
                    <div className="w-24 sm:w-28 md:w-32 bg-(--sw-surface-alt) border-r border-(--sw-border) flex flex-col">
                      {[
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center gap-1 py-4 px-2 text-center relative transition-colors ${isActive ? 'bg-(--sw-surface) text-primary' : 'text-(--sw-text-muted) hover:bg-(--sw-surface-alt)'
 }`}
                          >
                            {isActive && (
                    ...
                    <div ref={rightContentRef} className="flex-1 overflow-y-auto p-4 md:p-6">
                      ...
                      >
                        <h3 className="text-lg md:text-xl font-semibold text-(--sw-text) mb-4">Sort by</h3>
 ...
 : 'border-(--sw-border) hover:border-primary'
 }`}
                            >
                              <span className={`text-sm md:text-base font-medium ${sortBy === option.id ? 'text-green-600 dark:text-green-400' : 'text-(--sw-text-secondary)'}`}>
                                {option.label}
                              </span>
                            </button>
                          ))}
                      ...
                      <div
                        ...
                        className="space-y-4 mb-8"
                      >
                        <h3 className="text-lg md:text-xl font-semibold text-(--sw-text) mb-4">Estimated Time</h3>
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <Timer className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('under-30-mins') ? 'text-primary' : 'text-(--sw-text-secondary)'}`} strokeWidth={1.5} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('under-30-mins') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under 30 mins</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <Timer className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('delivery-under-45') ? 'text-primary' : 'text-(--sw-text-secondary)'}`} strokeWidth={1.5} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('delivery-under-45') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under 45 mins</span>
                          </button>
                        </div>
                      </div>
                      ...
                      <div
                        ...
                        className="space-y-4 mb-8"
                      >
                        <h3 className="text-lg md:text-xl font-semibold text-(--sw-text) mb-4">Restaurant Rating</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <Star className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('rating-35-plus') ? 'text-primary fill-primary' : 'text-(--sw-text-muted)'}`} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('rating-35-plus') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Rated 3.5+</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <Star className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('rating-4-plus') ? 'text-primary fill-primary' : 'text-(--sw-text-muted)'}`} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('rating-4-plus') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Rated 4.0+</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <Star className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('rating-45-plus') ? 'text-primary fill-primary' : 'text-(--sw-text-muted)'}`} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('rating-45-plus') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Rated 4.5+</span>
                          </button>
                        </div>
                      </div>
                      ...
                      <div
                        ...
                        className="space-y-4 mb-8"
                      >
                        <h3 className="text-lg md:text-xl font-semibold text-(--sw-text) mb-4">Distance</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <MapPin className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('distance-under-1km') ? 'text-primary' : 'text-(--sw-text-secondary)'}`} strokeWidth={1.5} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('distance-under-1km') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under 1 km</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <MapPin className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('distance-under-2km') ? 'text-primary' : 'text-(--sw-text-secondary)'}`} strokeWidth={1.5} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('distance-under-2km') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under 2 km</span>
                          </button>
                        </div>
                      </div>
                      ...
                      <div
                        ...
                        className="space-y-4 mb-8"
                      >
                        <h3 className="text-lg md:text-xl font-semibold text-(--sw-text) mb-4">Dish Price</h3>
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('price-under-200') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under ₹200</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('under-250') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under ₹250</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('price-under-500') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Under ₹500</span>
                          </button>
                        </div>
                      </div>
                      ...
                      <div
                        ...
                        className="space-y-4 mb-8"
                      >
                        <h3 className="text-lg md:text-xl font-semibold text-(--sw-text) mb-4">Offers</h3>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <BadgePercent className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('flat-50-off') ? 'text-primary' : 'text-(--sw-text-secondary)'}`} strokeWidth={1.5} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('flat-50-off') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Flat 50% OFF</span>
                          </button>
                          <button
 ...
 : 'border-(--sw-border) hover:border-primary'
                          ...
                          >
                            <BadgePercent className={`h-6 w-6 md:h-7 md:w-7 ${activeFilters.has('price-match') ? 'text-primary' : 'text-(--sw-text-secondary)'}`} strokeWidth={1.5} />
                            <span className={`text-sm md:text-base font-medium ${activeFilters.has('price-match') ? 'text-primary' : 'text-(--sw-text-secondary)'}`}>Price Match</span>
                          </button>
                        </div>
                      </div>
                      ...
                      {activeFilterTab === 'trust' && (
                        <div className="space-y-4">
                          <h3 className="text-lg md:text-xl font-semibold text-(--sw-text)">Trust Markers</h3>
                          <div className="flex flex-col gap-3 md:gap-4">
                            <button className="px-4 md:px-5 py-3 md:py-4 rounded-xl border border-(--sw-border) hover:border-primary text-left transition-colors">
                              <span className="text-sm md:text-base font-medium text-(--sw-text-secondary)">Top Rated</span>
                            </button>
                            <button className="px-4 md:px-5 py-3 md:py-4 rounded-xl border border-(--sw-border) hover:border-primary text-left transition-colors">
                              <span className="text-sm md:text-base font-medium text-(--sw-text-secondary)">Trusted by 1000+ users</span>
                            </button>
                          </div>
                        </div>
                      )}
                  ...
                  {/* Footer */}
                  <div className="flex items-center gap-4 px-4 md:px-6 py-4 border-t border-(--sw-border) bg-(--sw-surface)">
                    <button
                      onClick={() => setIsFilterOpen(false)}
                      className="flex-1 py-3 md:py-4 text-center font-semibold text-(--sw-text-secondary) text-sm md:text-base"
                    >
                      Close
                    </button>
                    <button
 ...
 : 'bg-(--sw-surface-sunken) text-(--sw-text-muted)'
 }`}
                    >
