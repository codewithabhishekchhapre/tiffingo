...
import { ArrowLeft, Clock, Search, SlidersHorizontal, ChevronDown, Bookmark, BadgePercent, Mic, Grid2x2 } from "lucide-react"
...
export default function SearchResults() {
  ...
  return (
    <div className={`min-h-screen bg-(--sw-surface) ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
      {/* Sticky Header */}
      <div className="sticky top-0 z-20 bg-(--sw-surface) shadow-sm">
        <div className="max-w-7xl mx-auto">
          {/* Search Bar with Back Button */}
          <div className="flex items-center gap-2 px-3 sm:px-4 md:px-6 lg:px-8 py-3 md:py-4 border-b border-(--sw-border)">
            <button
            ...
            <form onSubmit={handleSearch} className="flex-1 relative">
              ...
              <Input
                ...
                className="pl-10 pr-10 h-11 rounded-lg border-(--sw-border-strong) bg-(--sw-surface-alt) focus:bg-(--sw-surface) dark:focus:bg-[#2a2a2a] focus:border-(--sw-border-strong) text-sm dark:text-white placeholder:text-gray-600 dark:placeholder:text-gray-400"
              />
              <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2">
                <Mic className="h-4 w-4 text-(--sw-text-muted)" />
              </button>
            </form>
          </div>
          ...
          <div
            ...
            className="flex gap-3 sm:gap-4 lg:gap-5 overflow-x-auto scrollbar-hide px-4 sm:px-6 md:px-8 lg:px-10 py-3 md:py-4 bg-(--sw-surface) border-b border-(--sw-border)"
            style={{
              scrollbarWidth: "none",
          ...
          >
            {categories.map((cat) => {
              ...
              return (
                ...
                >
                  {isAllCategory ? (
                    <div className={`w-16 h-16 rounded-full border-2 transition-all flex items-center justify-center ${isSelected ? 'border-primary dark:border-primary shadow-lg bg-red-50 dark:bg-primary/20' : 'border-(--sw-border) bg-(--sw-surface) '}`}>
                      <Grid2x2 className={`h-6 w-6 ${isSelected ? 'text-primary' : 'text-(--sw-text-muted)'}`} />
                    </div>
                  ) : cat.image ? (
                    ...
                    <div className={`w-16 h-16 rounded-full bg-(--sw-surface-alt) flex items-center justify-center border-2 transition-all ${isSelected ? 'border-primary dark:border-primary shadow-lg bg-red-50 dark:bg-primary/20' : 'border-transparent'
 }`}>
                      <span className="text-xl">???</span>
                    </div>
                  )}
                  <span className={`text-xs font-medium whitespace-nowrap ${isSelected ? 'text-primary dark:text-primary' : 'text-(--sw-text-secondary)'
 }`}>
                    {cat.name}
                  </span>
                </button>
          ...
          {/* Filters */}
          <div
            className="flex items-center gap-2 sm:gap-3 lg:gap-4 overflow-x-auto scrollbar-hide px-4 sm:px-6 md:px-8 lg:px-10 py-3 md:py-4 bg-(--sw-surface) border-b border-(--sw-border)"
            style={{
              scrollbarWidth: "none",
              msOverflowStyle: "none",
            }}
          >
            ...
            <Button
              variant="outline"
              className="h-9 px-3 rounded-lg flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 font-medium bg-(--sw-surface) border border-(--sw-border) hover:bg-(--sw-surface-alt) text-(--sw-text-secondary)"
            >
              <SlidersHorizontal className="h-4 w-4" />
              <span className="text-sm font-bold text-(--sw-text)">Filters</span>
 ...
 ? 'bg-primary text-white border-primary hover:bg-primary dark:bg-primary dark:hover:bg-primary'
 : 'bg-(--sw-surface) border border-(--sw-border) hover:bg-(--sw-surface-alt) text-(--sw-text-secondary)'
 }`}
                >
                  ...
                  <span className={`text-sm font-bold ${isActive ? 'text-white' : 'text-(--sw-text)'}`}>{filter.label}</span>
                </Button>
              )
            })}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4 sm:py-6 md:py-8 lg:py-10 space-y-6 md:space-y-8 lg:space-y-10">
        ...
        {!showRestaurantSkeleton && filteredRecommended.length > 0 && (
          <section>
            <h2 className="text-xs sm:text-sm font-semibold text-(--sw-text-muted) tracking-widest uppercase mb-4">
              RECOMMENDED FOR YOU
            </h2>
                ...
                return (
                  ...
                  >
                    <div className={`group ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
                      {/* Image Container */}
                      <div className="relative aspect-square rounded-xl overflow-hidden mb-2 bg-(--sw-surface-sunken)">
                        {restaurant.image ? (
                          <img
                        ...
                        {restaurant.offer && (
                          <div className="absolute top-1.5 left-1.5 bg-gradient-to-r from-primary to-primary text-white text-[10px] font-semibold px-1.5 py-0.5 rounded">
                            {restaurant.offer}
                          </div>
                        )}
                      </div>
                      ...
                      {/* Restaurant Info */}
                      <h3 className="font-semibold text-(--sw-text) text-xs line-clamp-1">
                        {restaurant.name}
                      </h3>
                      {restaurant.deliveryTime && (
                        <div className="flex items-center gap-1 text-(--sw-text-muted) text-[10px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5 lg:gap-6">
            {nonRepeatedAllRestaurants.map((restaurant) => {
              ...
              return (
                <Link key={restaurant.id} to={`/user/restaurants/${restaurant.slug || restaurantSlug}`} className="h-full flex">
                  <Card className={`overflow-hidden cursor-pointer border-0 dark:border-gray-800 group bg-(--sw-surface) shadow-md hover:shadow-xl transition-all duration-300 py-0 rounded-md flex flex-col h-full w-full ${shouldShowGrayscale ? 'grayscale opacity-75' : ''
 }`}>
                    {/* Image Section */}
                    <div className="relative h-44 sm:h-52 md:h-60 lg:h-64 xl:h-72 w-full overflow-hidden rounded-t-md flex-shrink-0 bg-(--sw-surface-sunken)">
                      {restaurant.image ? (
                        <img
                          src={restaurant.image}
                          alt={restaurant.name}
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-(--sw-surface-sunken)">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="absolute top-3 right-3 h-9 w-9 bg-white/90 dark:bg-[#1a1a1a]/90 backdrop-blur-sm rounded-lg hover:bg-(--sw-surface) dark:hover:bg-[#2a2a2a] transition-colors"
                        onClick={(e) => {
                          e.preventDefault()
                          e.stopPropagation()
                      ...
                      >
                        <Bookmark className={`h-5 w-5 ${isFavorite ? "fill-gray-800 dark:fill-gray-200 text-(--sw-text)" : "text-(--sw-text-secondary)"}`} strokeWidth={2} />
                      </Button>
                    </div>
                    ...
                    <CardContent className="p-3 sm:p-4 lg:p-5 flex flex-col flex-grow">
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2 lg:mb-3">
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-(--sw-text) line-clamp-1 lg:line-clamp-2">
                            {restaurant.name}
                          </h3>
                        </div>
                        {restaurant.rating && (
                      ...
                      {(restaurant.deliveryTime || restaurant.distance) && (
                        <div className="flex items-center gap-1 text-sm lg:text-base text-(--sw-text-muted) mb-2 lg:mb-3">
                          {restaurant.deliveryTime && (
                            <>
            ...
            {nonRepeatedAllRestaurants.length === 0 && (
              <div className="text-center py-12">
                <p className="text-(--sw-text-muted)">
                  {query
