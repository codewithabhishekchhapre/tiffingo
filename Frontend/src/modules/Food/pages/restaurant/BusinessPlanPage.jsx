  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4">
                  <span className="bg-primary text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest">Active Plan</span>
                </div>
            ...
            sortedPlans.map((plan) => {
              ...
              return (
                <motion.div 
                  key={plan._id} 
                  whileHover={{ y: -5 }}
                  className={`bg-white rounded-3xl p-6 border-2 transition-all shadow-sm flex flex-col ${isCurrentPlan ? 'border-primary' : 'border-slate-100'}`}
                >
                  ...
                  {plan.durationUnit === 'DAY' ? (
                    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-5 relative overflow-hidden group/info">
                      <div className="flex items-start gap-3 relative z-10">
                        <div className="w-8 h-8 rounded-xl bg-white border border-slate-200 flex items-center justify-center shrink-0 shadow-sm">
                          <Info className="w-4 h-4 text-slate-400 group-hover/info:text-primary transition-colors" />
                        </div>
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <p className="font-bold text-slate-900 text-sm">Auto Activated</p>
                            <span className="bg-primary/10 text-primary text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider">Smart Flow</span>
                          </div>
                    ...
                    <Button 
                      ...
                      className={`w-full h-12 rounded-2xl font-bold text-sm tracking-wide transition-all ${(isCurrentPlan || isLocked) ? 'bg-slate-100 text-slate-400' : 'bg-primary hover:bg-[#E64D02] text-white shadow-lg shadow-primary/20'}`}
                    >
                      {purchasing === plan._id ? (
      ...
      <Modal
        ...
        onClose={() => !cancellingSub && setShowCancelModal(false)}
        dismissible={!cancellingSub}
        size="md"
        showClose={false}
        footer={
          <div className="space-y-4">
            <Button
              ...
              className="w-full h-14 bg-red-600 hover:bg-red-700 text-white rounded-2xl text-sm font-black shadow-xl shadow-red-200 dark:shadow-none active:scale-95 transition-all flex items-center justify-center gap-2.5 disabled:opacity-40"
            >
            ...
            <button
              ...
              disabled={cancellingSub}
              className="w-full py-1 text-slate-400 dark:text-gray-500 font-bold text-[10px] uppercase tracking-[0.2em] hover:text-slate-600 dark:hover:text-gray-300 transition-colors"
            >
              Keep Auto-Renewal
            </button>
          </div>
      ...
      >
        ...
        <div className="flex items-center gap-4 mb-6 pt-2">
          <div className="w-12 h-12 bg-red-50 dark:bg-red-900/20 rounded-2xl flex items-center justify-center text-red-600 dark:text-red-400 shrink-0 border border-red-100 dark:border-red-800/40">
            <AlertCircle className="w-6 h-6" />
          </div>
          <div className="text-left">
            <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-1.5">Cancel Auto-Renewal?</h3>
            <p className="text-[10px] text-slate-400 dark:text-gray-500 font-bold uppercase tracking-wider">Stop future auto-debits</p>
          </div>
        </div>

        <div className="bg-slate-50 dark:bg-gray-800/40 rounded-2xl p-4 border border-slate-100 dark:border-gray-800">
          <p className="text-slate-600 dark:text-gray-300 text-xs leading-relaxed font-bold">
            Your current plan remains <span className="text-green-600 dark:text-green-400 font-black">active</span> until its expiry date. You will not lose paid access, but no further payments will be automatically debited.
          </p>
        </div>
      </Modal>
    </div>
