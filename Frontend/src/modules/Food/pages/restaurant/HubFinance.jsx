        ...
        {activeTab === "payouts" && (
          <div className="space-y-5">
            ...
            <div className="bg-gradient-to-br from-primary to-[#e05e00] rounded-2xl p-5 text-white">
              {loading ? (
                <div className="py-8 text-center text-orange-200">Loading…</div>
              ) : (
                <>
                  <p className="text-xs text-orange-200 font-medium mb-1">Available for withdrawal</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Current Cycle</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">
                  ₹{(financeData?.currentCycle?.totalEarnings || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
                <p className="text-[10px] text-gray-400 mt-1">{financeData?.currentCycle?.totalOrders || 0} orders</p>
              </div>
              <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Pending Payout</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">
                  ₹{(financeData?.earnings?.pendingPayout || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
                <p className="text-[10px] text-gray-400 mt-1">Unsettled share</p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm font-bold text-gray-900 dark:text-white">Referral earnings</p>
                <button onClick={() => navigate("/restaurant/refer-earn")} className="text-xs font-semibold text-primary">Refer more</button>
              </div>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">From {referralStats?.referralCount || 0} approved referrals</p>
                </div>
                <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center">
                  <Gift className="w-6 h-6 text-primary" />
                </div>
              </div>
              <p className="text-[11px] text-gray-400 mt-3 pt-3 border-t border-gray-50 dark:border-gray-800">
            ...
            <div className="bg-white dark:bg-[#111] rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-50 dark:border-gray-800/60 flex items-center justify-between">
                <p className="text-sm font-bold text-gray-900 dark:text-white">Withdrawal requests</p>
                <button onClick={() => navigate("/restaurant/withdrawal-history")} className="text-xs font-semibold text-primary">View all</button>
              </div>
              <div className="p-4">
                {loadingWithdrawals ? (
                  <div className="py-6 text-center text-sm text-gray-400">Loading…</div>
                ) : withdrawalRequests.length === 0 ? (
                  <div className="py-6 text-center text-sm text-gray-400">No withdrawal requests found.</div>
                ) : (
                  <div className="space-y-2">
                    {withdrawalRequests.slice(0, 5).map((request, index) => {
                      const status = formatWithdrawalStatus(request?.status)
                      return (
                        <div key={request?._id || request?.id || index} className="flex items-center justify-between py-2.5 border-b border-gray-50 dark:border-gray-800/60 last:border-0">
                          <div>
                            <p className="text-sm font-bold text-gray-900 dark:text-white">
                              ₹{Number(request?.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </p>
                            <p className="text-xs text-gray-400 mt-0.5">{formatDateTime(request?.createdAt || request?.requestedAt)}</p>
                          </div>
                          <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold ${getWithdrawalStatusClass(request?.status)}`}>{status}</span>
                        </div>
                      )
                    })}
                  </div>
