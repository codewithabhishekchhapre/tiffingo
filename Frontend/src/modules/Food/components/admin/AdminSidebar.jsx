...
export default function AdminSidebar({ isOpen = false, onClose, onCollapseChange }) {
  ...
  const [logoUrl, setLogoUrl] = useState(() => getAppLogo('admin') || '/food/tiffingo-logo.png')
  const [faviconUrl, setFaviconUrl] = useState(() => getAppFavicon('admin') || '/food/tiffingo-icon.png')
    ...
    const loadLogo = async () => {
      try {
        ...
        if (cached) {
          ...
          setLogoUrl(adminLogo || '/food/tiffingo-logo.png')
      ...
      return (
        <Link
          ...
          className={cn(
            ...
            isActive(item.path)
              ? "bg-[#FFF3EB] text-primary border border-[#FFF3EB]/30 font-semibold shadow-xs"
        ...
        >
          <Icon className={cn(
            ...
            isActive(item.path) ? "text-primary scale-110" : "text-[#5C5247] group-hover:text-[#1A1A1A]"
          )} />
          {!isCollapsed && (
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive(subItem.path, allSubPaths)
                        ? "bg-[#FFF3EB] text-primary font-semibold shadow-xs"
                        : "text-[#5C5247] hover:bg-[#FAF7F2] hover:text-[#1A1A1A]"
                    )}
                  ...
                  >
                    <span className={cn(
                      ...
                      isActive(subItem.path, allSubPaths) ? "bg-primary scale-125" : "bg-[#5C5247]"
                    )}></span>
                  ...
                  {visibleModuleTabs.map(tab => (
                    <button
                      ...
                      className={cn(
                        ...
                        tab.active
                          ? "bg-primary text-white shadow-[0_4px_12px_rgba(255, 106, 0,0.2)]"
          ...
          {!isCollapsed && (
            <div className="relative animate-[slideIn_0.4s_ease-out_0.2s_both]">
              ...
              <Input
                ...
                className={cn(
                  "w-full pl-9 py-2 bg-[#ffffffcc] border border-[#EDE8E0] rounded-lg text-sm text-[#1A1A1A] placeholder:text-[#7C7062] focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40 transition-all duration-200 text-left",
