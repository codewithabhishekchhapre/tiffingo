...
const OptimizedImage = React.memo(({
  ...
  onError,
  backendOrigin = "",
  fallback = null, // Rendered instead of the grey tile when src is unusable/broken
  ...props
}) => {
  ...
  // A CMS row can hand us "", null, an object, or the literal strings a bad
  // serialisation leaves behind — none of those are worth an <img> request.
  const hasUsableSrc = useMemo(() => {
    if (typeof src !== 'string') return false
    const trimmed = src.trim()
    if (!trimmed || /^(null|undefined|false|nan)$/i.test(trimmed)) return false
    return Boolean(resolvedSrc)
  }, [src, resolvedSrc])

  // A new src deserves a fresh attempt — otherwise one broken URL pins the
  // component to its error state for the rest of the carousel.
  useEffect(() => {
    setHasError(false)
    setIsLoaded(false)
  }, [resolvedSrc])
  ...
  // Caller supplied its own placeholder (e.g. a gradient banner slide): use it
  // for both "nothing to show" and "the image failed to load".
  if (fallback && (!hasUsableSrc || hasError)) {
