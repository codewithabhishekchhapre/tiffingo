...
const RestaurantImageCarousel = React.memo(
  ({
    ...
    priority = false,
    backendOrigin = "",
    topRadiusClassName = "rounded-t-md",
    // Swiggy cards use a shorter 4:3-ish crop than the original tall hero.
    // Callers opt in; existing callers keep the previous fixed heights.
    heightClassName = "h-48 sm:h-56 md:h-60 lg:h-64 xl:h-72",
  }) => {
    ...
    return (
      <div
        className={`relative ${heightClassName} w-full overflow-hidden ${topRadiusClassName} flex-shrink-0 group`}
