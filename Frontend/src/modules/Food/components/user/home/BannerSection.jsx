...
// The cut-out artwork on the right of the overlay layout. A missing or broken
// file leaves a hole next to the copy, so swap it for the gradient panel.
const BannerGraphic = ({ src, index }) => {
  const [failed, setFailed] = useState(false);

  useEffect(() => setFailed(false), [src]);

  if (!src || failed) {
    return <BannerFallback index={index} className="absolute inset-0 rounded-l-(--sw-radius-card)" />;
  }
  ...
  return (
    <img
      ...
      className="max-h-full w-auto object-contain object-bottom drop-shadow-2xl translate-y-2 sm:translate-y-4 translate-x-2 sm:translate-x-4"
      alt="Banner Graphic"
    />
  );
};
...
const BannerSection = memo(({
  ...
  // No artwork configured at all: the slot is already laid out by the parent,
  // so fill it with the gradient rather than leaving an empty bordered box.
  if (!heroBannerImages || heroBannerImages.length === 0) {
    return <BannerFallback index={0} className="h-full w-full" />;
  }
                      ...
                      <div className="relative h-full w-full flex items-center justify-between px-2 sm:px-6">
                        ...
                        <div className="relative z-10 flex flex-col justify-center h-full text-white w-[60%] sm:w-[65%] mt-2 pl-4 sm:pl-8">
                          ...
                          <motion.div
                            ...
                            className="w-fit"
                          >
                            {/* NB: the previous shadow-[...] arbitrary value contained spaces,
                                so Tailwind discarded the class and it never rendered. */}
                            <button className="sw-pressable flex items-center gap-1 rounded-(--sw-radius-control) bg-primary px-4 py-2 font-bold text-white shadow-(--sw-shadow-md) transition-all hover:bg-(--sw-primary-hover)">
                        ...
                        >
                          <BannerGraphic src={image} index={index} />
                        </motion.div>
                      </div>
                    )}
                    {!hideOverlay ? (
