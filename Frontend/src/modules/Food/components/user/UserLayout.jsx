...
export default function UserLayout({ children }) {
  ...
  return (
    <div className="food-theme-scope min-h-screen bg-(--sw-bg) transition-colors duration-200">
      <CartProvider>
        <ProfileProvider>
          <OrdersProvider>
              <SearchOverlayProvider>
                <LocationSelectorProvider>
                  ...
                  <LocationPrompt />
                  {/* The desktop navbar is a single 76px row, so reserve exactly that. */}
                  <main className={showFoodBottomNav ? "md:pt-[76px]" : ""}>
                    {children || <Outlet />}
                  </main>
                  {/* Footer runs on every breakpoint; it reserves room for the bottom nav itself. */}
                  {showFoodBottomNav && <Footer />}
                  {showFoodBottomNav && <BottomNavigation />}
                  {showFoodBottomNav && <DesktopBottomNav />}
