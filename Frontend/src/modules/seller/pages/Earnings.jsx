  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <BlurFade delay={0.2}>
          <Card className="bg-gradient-to-br from-red-600 to-red-700 text-white border-none shadow-lg h-full">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-red-100 font-medium">Total Net Earnings</p>
                <h3 className="text-4xl font-bold mt-2">₹{Number(data?.balances?.totalNetEarnings ?? 0).toLocaleString()}</h3>
                <p className="text-xs text-red-200 mt-2">After all deductions</p>
              </div>
              <div className="p-3 bg-white/20 rounded-xl">
                <IndianRupee className="h-8 w-8 text-white" />
              </div>
            </div>
            <div className="mt-8 flex items-center text-red-100 bg-white/10 w-fit px-3 py-1 rounded-full text-sm">
