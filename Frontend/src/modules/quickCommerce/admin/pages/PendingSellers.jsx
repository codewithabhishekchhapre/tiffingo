...
const buildDocs = (seller) => {
  ...
  if (seller?.shopInfo?.businessType === "Pharmacy") {
    if (seller?.documents?.medicalLicenseNumber || seller?.documents?.medicalLicenseImage) docs.push('Medical');
  } else {
  ...
  const filteredSellers = useMemo(() => {
    ...
    if (typeFilter !== 'All') {
      result = result.filter(seller => {
        const type = seller.shopInfo?.businessType || '';
        if (typeFilter === 'Pharmacy') return type === 'Pharmacy';
        if (typeFilter === 'Quick Commerce') return type !== 'Pharmacy';
        return true;
      });
    }
                  ...
                  <tr key={seller._id} className="hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        seller.shopInfo?.businessType === 'Pharmacy' ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"
                      )}>
                        {seller.shopInfo?.businessType === 'Pharmacy' ? 'Pharmacy' : 'Quick Commerce'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[0.95fr_1.4fr]">
                  ...
                  <div className="p-6 lg:p-8">
                    <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <div className="grid gap-3 md:grid-cols-2">
                            {[
                              ...
                              ...(viewingSeller.shopInfo?.businessType === "Pharmacy" ? [
                                ['Medical License', viewingSeller.documents?.medicalLicenseNumber],
                                ['Medical Expiry', viewingSeller.documents?.medicalLicenseExpiry ? new Date(viewingSeller.documents.medicalLicenseExpiry).toLocaleDateString('en-IN') : null],
                              ] : [
                      ...
                      {(viewingSeller.bankInfo?.upiQrImage || viewingSeller.documents?.shopLicenseImage || viewingSeller.documents?.medicalLicenseImage || viewingSeller.documents?.fssaiImage) && (
                        <div className="space-y-4 pt-2">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            {viewingSeller.documents?.medicalLicenseImage && (
                              <div className="space-y-3">
                                <p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">Medical License</p>
                                <div className="group relative aspect-square w-full overflow-hidden rounded-3xl border-2 border-slate-100 bg-slate-50 transition-all hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5">
                                  <img 
                                    src={viewingSeller.documents.medicalLicenseImage} 
                                    alt="Medical License" 
                                    className="h-full w-full object-contain p-4 transition-transform duration-500 group-hover:scale-105"
                                  />
                                  <a 
                                    href={viewingSeller.documents.medicalLicenseImage} 
                                    target="_blank" 
                                    rel="noreferrer"
                                    className="absolute inset-0 flex items-center justify-center bg-slate-900/0 backdrop-blur-0 opacity-0 transition-all duration-300 group-hover:bg-slate-900/40 group-hover:backdrop-blur-sm group-hover:opacity-100"
                                  >
