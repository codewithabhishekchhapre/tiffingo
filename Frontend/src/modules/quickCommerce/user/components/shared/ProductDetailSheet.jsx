...
const ProductDetailSheet = () => {
    ...
    // Memoised product details grid (changes only when supportEmail changes)
    const productDetails = useMemo(() => PRODUCT_DETAILS_TEMPLATE(supportEmail), [supportEmail]);
    ...
    }, [cart, selectedProduct, productId]);

    const quantity = cartItem?.quantity ?? 0;
    const isWishlisted = useMemo(
        () => (selectedProduct ? isInWishlist(productId) : false),
        [selectedProduct, isInWishlist, productId]
    );
    ...
    useEffect(() => {
        if (!selectedProduct) return;
        setSelectedVariant(
            selectedProduct.variants?.length > 0 ? selectedProduct.variants[0] : null
        );
    ...
    }, [quantity, removeFromCart, updateQuantity, productId]);
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    ...
                                    {quantity > 0 ? (
                                        <div className="flex items-center gap-3 bg-white border-2 border-green-500 rounded-xl p-1.5 shadow-lg shadow-green-100 flex-1 justify-between max-w-[180px]">
                                            ...
                                            <motion.button whileTap={{ scale: 0.9 }} onClick={handleIncrement} className="w-10 h-10 bg-[#0c831f] rounded-lg flex items-center justify-center text-white hover:bg-[#0b721b] transition-colors shadow-md shadow-green-100">
