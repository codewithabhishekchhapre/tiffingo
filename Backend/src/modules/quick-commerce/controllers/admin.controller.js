...
const toCategory = (category) => ({
  ...
  businessType: category.businessType || 'quick_commerce',
...
const buildQuickAdminOrderResponse = (order, sellerMap = {}, sellerOrderMap = {}) => {
  ...
  return {
    ...
    orderType: (order.orderType === "mixed") ? "mixed" : (isPharmacy ? "pharmacy" : (order.orderType || "quick")),
    total: payableTotal,
    amount: payableTotal,
    ...
    customer: resolveQuickOrderCustomer(order, sellerOrder),
    seller: seller
      ? {
          _id: seller._id,
          shopName: seller.shopName || seller.name || 'Store',
...
export const createCategory = async (req, res) => {
  ...
  const category = await QuickCategory.create({
    ...
    type: type || 'header',
    businessType: type === 'header' ? (businessType || 'quick_commerce') : undefined,
...
export const updateCategory = async (req, res) => {
  ...
  if (category.type === 'header' && businessType !== undefined) category.businessType = businessType || 'quick_commerce';
  if (status !== undefined) {
...
export const getAdminProducts = async (req, res) => {
  ...
  if (businessType && businessType !== 'all') {
    ...
    } else if (businessType === 'quick_commerce') {
      headerQuery.$and = [
        { businessType: { $ne: 'pharmacy' } },
        { name: { $not: { $regex: /pharmacy|medicine/i } } }
      ];
    } else {
...
const parsePharmacyDetailsBody = (value) => {
...
export const updateProduct = async (req, res) => {
  ...
  const pharmacyDetails = parsePharmacyDetailsBody(body.pharmacyDetails);
  if (pharmacyDetails) product.pharmacyDetails = pharmacyDetails;
  if (images.mainImage) {
...
export const getAdminOrderById = async (req, res) => {
  ...
  const [sellers, sellerOrders] = await Promise.all([
    Seller.find({ _id: { $in: sellerIds } }).select('_id shopName name location shopInfo.businessType').lean(),
    SellerOrder.find({ orderId: order.orderId }).select('_id orderId status workflowStatus customer address cancellationReason').lean(),
  ]);
